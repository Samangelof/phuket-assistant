...
GPT_ALL_MODELS = GPT_3_MODELS + GPT_3_16K_MODELS + GPT_4_MODELS + GPT_4_32K_MODELS + \
    GPT_4_VISION_MODELS + GPT_4_128K_MODELS + GPT_4O_MODELS + O_MODELS
...
class OpenAIHelper:
    ...
    def set_prompt(self, new_prompt: str):
        ...
        self.assistant_prompt = new_prompt
    ...
    async def get_chat_response(self, chat_id: int, query: str) -> tuple[str, str]:
        ...
        show_plugins_used = len(
            plugins_used) > 0 and self.config['show_plugins_used']
        plugin_names = tuple(self.plugin_manager.get_plugin_source_name(
            plugin) for plugin in plugins_used)
    ...
    async def __common_get_chat_response(self, chat_id: int, query: str, stream=False):
        ...
        try:
            ...
            max_tokens_str = 'max_completion_tokens' if self.config[
                'model'] in O_MODELS else 'max_tokens'
        ...
        #! Подмешиваю дату и время в системное сообщение
            ...
            common_args = {
                ...
                'messages': messages_with_datetime,
    ...
    def reset_chat_history(self, chat_id, content=''):
        ...
        self.conversations[chat_id] = [
            {"role": "assistant" if self.config['model'] in O_MODELS else "system", "content": content}]
                    ...
                    if isinstance(value, str):
                        ...
                        for message1 in value:
                            if message1['type'] == 'image_url':
                                image = decode_image(
                                    message1['image_url']['url'])
                                num_tokens += self.__count_tokens_vision(image)
                            else:
                                num_tokens += len(
                                    encoding.encode(message1['text']))
                else:
                    num_tokens += len(encoding.encode(value))
                    if key == "name":
