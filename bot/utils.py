...
#* ------------------------------------------------------
#* Общий модуль для работы с prompts.json
PROMPTS_FILE_PATH = os.path.join(os.path.dirname(__file__), 'prompts.json')

def load_prompts(raise_if_not_found=False):
    """
    Загружает промпты из JSON файла
    :param raise_if_not_found: Если True - вызывает исключение при отсутствии файла
    :return: Словарь с промптами или пустой словарь, если файл не найден
    """
    if not os.path.exists(PROMPTS_FILE_PATH):
        if raise_if_not_found:
            raise FileNotFoundError(f"Файл {PROMPTS_FILE_PATH} не найден!")
        return {}
...
def save_prompts(prompts):
    ...
    :param prompts: Словарь с промптами для сохранения
    """
...
def is_admin(config, user_id: int, log_no_admin=False) -> bool:
    ...
    if not isinstance(config, dict):
        logging.error(f"is_admin called with invalid config type: {type(config)}")
        return False
