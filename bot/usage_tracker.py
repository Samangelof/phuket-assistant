...
class UsageTracker:
    ...
    def __init__(self, user_id, user_name, logs_dir="usage_logs"):
        ...
        if os.path.isfile(self.user_file):
            ...
            if 'message_logs' not in self.usage:
                self.usage['message_logs'] = {}
        else:
    ...
    def add_message_log(self, message_data: dict):
        """Сохраняет данные сообщения в JSON с кодировкой UTF-8."""
        date_str = str(date.today())
        
        if date_str not in self.usage["message_logs"]:
            self.usage["message_logs"][date_str] = []
            
        self.usage["message_logs"][date_str].append(message_data)
        
        with open(self.user_file, "w", encoding='utf-8') as outfile:
            json.dump(self.usage, outfile, indent=2, ensure_ascii=False)
